...
import sys
import os
from LEGEND import bot as javes
...
from LEGEND import CMD_HELP, LOGS
...
async def upstream(ups):
    ...
    try:
        ...
        if conf != "now":
            ...
            \nBut we can fix that by force updating the LEGEND using .update now.`"
            )
            return
        repo = Repo.init()
    ...
    if force_update:
        await ups.edit(
            '`Force-Syncing to latest stable LEGEND code, please wait...`')
    else:
    ...
    if HEROKU_APIKEY is not None:
        ...
        if not HEROKU_APPNAME:
            await ups.edit(
                '`Please set up the HEROKU_APPNAME variable to be able to update LEGEND.`'
            )
            repo.__del__()
            return
        ...
        if heroku_app is None:
            await ups.edit(
                f'{txt}\n`Invalid Heroku credentials for updating LEGEND dyno.`'
            )
            repo.__del__()
            return
