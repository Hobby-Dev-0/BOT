...
from LEGEND import LOGS, CMD_HELP, TEMP_DOWNLOAD_DIRECTORY
...
async def download(target_file):
    """ For .dl command, download files to the LEGEND's server. """
    await target_file.edit("Processing using LEGEND server ( ◜‿◝ )♡")
...
async def upload(u_event):
    """ For .upload command, allows you to upload a file from the LEGEND's server """
    await u_event.edit("Processing ...")
    input_str = u_event.pattern_match.group(1)
    if input_str in ("LEGEND.session", "config.env"):
