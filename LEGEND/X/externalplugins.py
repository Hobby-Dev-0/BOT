...
from LEGEND.utils import command, remove_plugin, load_module
from var import Var
from pathlib import Path
from LEGEND import LOAD_PLUG
import sys
import asyncio
import traceback
import os
...
async def install(event):
    ...
    for ixo in total_doxx:
        ...
        downloaded_file_name = await event.client.download_media(await borg.get_messages(chat, ids=mxo), "LEGEND/plugins/")
...
async def install(event):
    ...
    for ixo in total_doxx:
        ...
        downloaded_file_name = await event.client.download_media(
            await borg.get_messages(chat, ids=mxo), "LEGEND/plugins/"
        )
        if "(" not in downloaded_file_name:
            path1 = Path(downloaded_file_name)
            shortname = path1.stem
